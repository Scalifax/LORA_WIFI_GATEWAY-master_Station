...
static bool test_message = false; // Defines if a test message is going to be sent. True = send message
#define CONFIG_EXAMPLE_WIFI_SSID "Scalifax - Fundos"
    ...
    if (wifi_is_connected) // Verify if the WiFi is connected
    {
        if (xSemaphore != NULL)
        {
            ...
            if (xSemaphoreTake(xSemaphore, portMAX_DELAY) == pdTRUE)
            {
                ...
                {
                    printf("Wi-Fi tx successful, length: %d\n", len);
                }
...
static void wifi_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    else if (event_base == IP_EVENT)
    {
        ...
        case IP_EVENT_STA_GOT_IP:
            ...
            // Send a test message if test_message is true
            if (test_message)
            {
                const char *test_msg = "Test message from ESP32";
                wifi_tx_msg((void *)test_msg, strlen(test_msg));
                test_message = false; // set it to false so the message is only sent once
            }
